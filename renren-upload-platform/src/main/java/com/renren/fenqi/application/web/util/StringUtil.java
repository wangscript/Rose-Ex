...
//			System.out.println(cutStringRealLen("你好", 4, 0, 0));
			String [] strs = {"1"};
     ...
     * @return
     */
    public static String join(String[] resource, String separater,String prefix,String subfix) {
        if (CollectionUtil.isEmpty(resource)) {
        	resource = new String[0];
        }
        prefix = isBlank(prefix)?"":prefix.trim();
        subfix = isBlank(subfix)?"":subfix.trim();
        int len = resource.length;
        StringBuilder sb = new StringBuilder();
        if (len > 0) {
            sb.append(prefix).append(resource[0]).append(subfix);
        }
        for (int i = 1; i < len; i++) {
            sb.append(separater);
            sb.append(prefix).append(resource[i]).append(subfix);
        }
        return sb.toString();
    }
